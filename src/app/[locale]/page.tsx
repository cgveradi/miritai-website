...
function SystemMap() {
  return <div className="system-map" aria-hidden="true"><div className="map-grid"/><div className="orbit"><i/></div><div className="node node-a"><i/><i/><i/><i/><i/><i/></div><div className="node node-b"><i/><i/><i/></div><div className="node node-c"><i/><i/></div><div className="node node-d"><i/><i/><i/><i/><i/><i/></div><div className="wire wire-a"/><div className="wire wire-b"/><div className="wire wire-c"/></div>;
}
...
export default function Home() {
  ...
  const projects = [1, 2, 3].map((n) => ({ title: t(`project${n}Title`), text: t(`project${n}Text`) }));
  return <><Navbar/><main>
    ...
    <section className="section first-projects" id="work"><div className="wrap"><ScrollReveal><div className="section-head"><div><p className="eyebrow">{t("workEyebrow")}</p><h2>{t("workTitle")}</h2></div><p>{t("workIntro")}</p></div><div className="project-list">{projects.map((project,index)=><article key={project.title}><span>0{index+1}</span><h3>{project.title}</h3><p>{project.text}</p></article>)}</div></ScrollReveal></div></section>
  ...
  </main><footer><div className="wrap"><div className="footer-grid"><div><a className="logo" href="#top">MIRIT<span>AI</span></a><p>{t("footerText")}</p></div><div><h3>{t("footerServices")}</h3><a href="#services">{t("service1Title")}</a><a href="#services">{t("service2Title")}</a><a href="#services">{t("service3Title")}</a></div><div><h3>{t("footerStudio")}</h3><a href="#approach">{t("approachEyebrow")}</a><a href="#work">{t("workEyebrow")}</a></div><div><h3>{t("footerContact")}</h3><a href="mailto:hello@miritai.com">hello@miritai.com</a></div></div><div className="footer-bottom"><span>© 2026 MIRITAI. {t("rights")}</span><span>miritai.com</span></div></div></footer></>;
}
