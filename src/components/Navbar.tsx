...
export default function Navbar() {
  ...
  return <nav className="site-nav" aria-label="Main navigation"><div className="wrap nav-inner">
    ...
    <div className="nav-content"><div className="nav-links">{items.map((item) => <a key={item.href} href={item.href} aria-label={item.label}><DecodedNavLabel label={item.label}/></a>)}</div><div className="nav-utilities"><LanguageSwitcher/><ThemeToggle/><Link href="/contact" className="button nav-cta">{t("start")} <span aria-hidden="true">↗</span></Link><button className="menu-toggle" type="button" aria-expanded={open} aria-controls="mobile-navigation" aria-label={open ? "Close menu" : "Open menu"} onClick={() => setOpen(!open)}><span/><span/></button></div></div>
